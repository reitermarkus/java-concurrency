...
\begin{document}
    ...
    To find out how many threads a system can handle, we created an infinite loop which creates threads (each of which calls \textt{Thread.yield()} in an infinite loop) until an \texttt{OutOfMemoryError} occurs.
